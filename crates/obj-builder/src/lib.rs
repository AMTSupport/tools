...
use crate::error::{BuildResult, FillResult};
...
macro_rules! builder {
    ...
    (impl
        ...
        impl $crate::buildable::Buildable for $name {
        ...
        $crate::_paste::paste! {
            ...
            pub struct [< $name Builder >] {
                $($($required_ident: $crate::filler::FillableDefinition<$required_type>,)+)?
                $($($optional_ident: $crate::filler::FillableDefinition<$optional_type>,)+)?
            }
            ...
            impl $crate::Builder for [< $name Builder >] {
                type Buildable = $name;

                async fn fill<F: $crate::filler::Filler>(&mut self, filler: &F) -> $crate::error::FillResult<()> {
                    use $crate::Builder as _Builder;
                ...
                async fn build(self) -> Result<$name, $crate::error::BuildError> {
                    Ok($name {
    ...
    (@build_req $self:ident $field:ident) => {
        $self.$field.value.or($self.$field.default).ok_or_else(|| $crate::error::BuildError::MissingField { field: stringify!($field).to_string() })?
    };
    ...
    (@default $(#[$attr:meta])* $ident:ident: $ty:ty, $required:expr $(, $default:expr)?) => {{
        ...
        $crate::filler::FillableDefinition::<$ty> {
    ...
    (@try_fill [$self:ident, $filler:ident: $filler_generic:ident] $ident:ident: $type:ty) => {{
        ...
        use $crate::error::FillResult as _FillResult;
        use $crate::filler::FillableDefinition as _FillableDefinition;
        use $crate::filler::Filler as _Filler;
        ...
        let mut filled = {{
            ...
            $crate::conditional_call!(impl $type where T: Sized + Clone, F: _Filler | T: _FromStr + Clone + _Debug, F: _Filler {
                async fn call<'a>(
                    ...
                    def.value.as_ref().ok_or($crate::error::FillError::InvalidFiller {
                        field: stringify!($ident).to_string(),
                        filler: "FromStr".to_string(),
                    })
                } else {
                    Err($crate::error::FillError::InvalidDefinition)
                }
            });
        ...
        if let Err($crate::error::FillError::InvalidDefinition) = filled {
            filled = {
                use $crate::buildable::Buildable as _Buildable;
                ...
                $crate::conditional_call!(impl $type where T: Sized + Clone, F: _Filler | T: _Buildable + Clone + _Debug, F: _Filler {
                    async fn call<'a>(
                        ...
                        match builder.build().await {
                            Err(err) => Err($crate::error::FillError::Nested(err)),
                            Ok(value) => {
                                def.value.replace(value);
                                def.value.as_ref().ok_or($crate::error::FillError::InvalidFiller {
                                    field: stringify!($ident).to_string(),
