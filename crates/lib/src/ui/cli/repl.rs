...
crate::handler!(pub Repl<Option<O>> [
    ...
    async fn respond(&mut self, line: &str) -> CliResult<bool> {
        ...
        let mut matches = Self::Action::command().try_get_matches_from(&args).map_err(CliError::InvalidCommand)?;
        let parsed = Self::Action::from_arg_matches_mut(&mut matches).map_err(CliError::InvalidCommand)?;
